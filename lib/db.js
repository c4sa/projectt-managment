...
// role permissions — support ?roleId= filter for list
export const rolePermissionsHandler = makeTableHandler({
  table: 'role_permissions',
  toDb: toDb('role_permissions'),
  fromDb: toCamel,
  queryFilter: (q, req) => {
    const roleId = new URL(`http://x${req.url}`).searchParams.get('roleId');
    return roleId ? q.eq('role_id', roleId) : q;
  },
});
