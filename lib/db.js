  ...
  return async function handler(req, res) {
    ...
    return dbHandler(res, async () => {
      ...
      if (req.method === 'PUT' && id) {
        ...
        delete row.id;
        const { data, error } = await supabase.from(table).update(row).eq('id', id).select(select).maybeSingle();
        if (error) throw error;
        if (!data) return fail(res, 404, 'Not found');
        return ok(res, fromRow(data));
      }
...
const COLS = {
  projects: ['id','name','code','status','customer_id','project_manager','assistant_pm','assigned_manager_id','assigned_manager_name','team_members','start_date','end_date','description','location','budget','spent','contract_value','vat_status','contract_document','contract_link','archived','created_at'],
  ...
  documents: ['id','project_id','folder_id','name','type','file_url','file_path','file_size','mime_type','uploaded_by','uploaded_at','version','tags','permissions'],
  ...
  app_users: ['id','name','email','role','custom_role_id','status','created_at'],
  approval_workflows: ['id','type','name','description','steps','levels','auto_approval_threshold','escalation_timeout_hours','is_active','created_at','updated_at'],
  document_comments: ['id','document_id','user_id','user_name','message','edited','edited_at','created_at'],
  custom_roles: ['id','name','description','is_custom','created_at'],
  role_permissions: ['id','role_id','module','action','allowed'],
  document_folders: ['id','project_id','parent_id','name','description','color','created_by','created_at'],
  document_activities: ['id','document_id','folder_id','project_id','user_id','action','details','created_at'],
  project_chat_messages: ['id','project_id','user_id','user_name','message','edited','edited_at','created_at'],
};
...
// projects — custom fromDb/toDb for team_members JSONB (TeamMember[] with camelCase)
function projectsFromDb(row) {
  const out = toCamel(row);
  if (Array.isArray(out.teamMembers)) {
    out.teamMembers = out.teamMembers.map((m) => {
      if (typeof m === 'string') {
        return { id: m, userId: m, userName: '', userEmail: '', role: 'team_member', permissions: { expenses: false, income: false, budget: false, documents: true, tasks: true }, addedAt: '', addedBy: '' };
      }
      return toCamel(m);
    });
  } else if (out.teamMembers == null) {
    out.teamMembers = [];
  }
  return out;
}

function projectsToDb(body) {
  const snake = toSnake(body);
  const base = pick(snake, COLS.projects);
  if (Array.isArray(body?.teamMembers)) {
    base.team_members = body.teamMembers.map((m) => (typeof m === 'object' && m ? toSnake(m) : m));
  }
  return base;
}
...
// purchase orders — support optional ?projectId= filter
...
// variation orders — support optional ?projectId= and ?poId= filters
...
// vendor invoices — support optional ?projectId= and ?vendorId= filters
...
// customer invoices — support optional ?projectId= and ?customerId= filters
...
// payments — support optional ?projectId= filter
...
// approval workflows
export const approvalWorkflowsHandler = makeTableHandler({
  table: 'approval_workflows',
  toDb: toDb('approval_workflows'),
  fromDb: toCamel,
  orderBy: 'type',
  orderAsc: true,
});

// document comments — support ?documentId= filter for list
export const documentCommentsHandler = makeTableHandler({
  table: 'document_comments',
  toDb: toDb('document_comments'),
  fromDb: toCamel,
  orderBy: 'created_at',
  orderAsc: true,
  queryFilter: (q, req) => {
    const documentId = new URL(`http://x${req.url}`).searchParams.get('documentId');
    return documentId ? q.eq('document_id', documentId) : q;
  },
});

// project chat messages — support ?projectId= filter for list
export const projectChatMessagesHandler = makeTableHandler({
  table: 'project_chat_messages',
  toDb: toDb('project_chat_messages'),
  fromDb: toCamel,
  orderBy: 'created_at',
  orderAsc: true,
  queryFilter: (q, req) => {
...
// custom roles
export const customRolesHandler = makeTableHandler({
  table: 'custom_roles',
  toDb: toDb('custom_roles'),
  fromDb: toCamel,
  orderBy: 'name',
  orderAsc: true,
});

// role permissions — custom handler: no created_at (use role_id for order), POST upserts to avoid duplicate key
export async function rolePermissionsHandler(req, res) {
  const urlParts = req.url.split('?')[0].split('/').filter(Boolean);
  const id = urlParts.length >= 3 ? urlParts[urlParts.length - 1] : null;

  return dbHandler(res, async () => {
    if (req.method === 'GET' && !id) {
      let query = supabase.from('role_permissions').select('*').order('role_id', { ascending: true });
      const roleId = new URL(`http://x${req.url}`).searchParams.get('roleId');
      if (roleId) query = query.eq('role_id', roleId);
      const { data, error } = await query;
      if (error) throw error;
      return ok(res, (data ?? []).map(toCamel));
    }

    if (req.method === 'GET' && id) {
      const { data, error } = await supabase.from('role_permissions').select('*').eq('id', id).maybeSingle();
      if (error) throw error;
      if (!data) return fail(res, 404, 'Not found');
      return ok(res, toCamel(data));
    }

    if (req.method === 'POST') {
      const body = req.body;
      if (!body) return fail(res, 400, 'Request body is required');
      const row = nullifyEmptyFKs(pick(toSnake(body), COLS.role_permissions));
      if (!row.role_id || !row.module || !row.action) {
        return fail(res, 400, 'roleId, module, and action are required');
      }
      row.id = row.id || randomUUID();
      const { data: upserted, error } = await supabase
        .from('role_permissions')
        .upsert(row, { onConflict: 'role_id,module,action' })
        .select()
        .single();
      if (error) throw error;
      return ok(res, toCamel(upserted ?? row));
    }
    ...
    if (req.method === 'PUT' && id) {
      ...
      const row = nullifyEmptyFKs(pick(toSnake(body), COLS.role_permissions));
      delete row.id;
      const { data, error } = await supabase.from('role_permissions').update(row).eq('id', id).select('*').single();
      if (error) throw error;
      if (!data) return fail(res, 404, 'Not found');
      return ok(res, toCamel(data));
    }
...
// document folders — support ?projectId= filter for list
export const documentFoldersHandler = makeTableHandler({
  table: 'document_folders',
  toDb: toDb('document_folders'),
  fromDb: toCamel,
  orderBy: 'name',
  orderAsc: true,
  queryFilter: (q, req) => {
...
// document activities — support ?projectId= or ?documentId= filter for list
export const documentActivitiesHandler = makeTableHandler({
  table: 'document_activities',
  toDb: toDb('document_activities'),
  fromDb: toCamel,
  orderBy: 'created_at',
  orderAsc: false,
  queryFilter: (q, req) => {
